...
import { ingestEvents } from "@/lib/db.server";
import type { IngestEventsRequest, TimecodeEvent } from "@/lib/types";
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!payload) {
          return Response.json({ error: "Invalid /events payload" }, { status: 400, headers: corsHeaders() });
        }
