...
import { queryDailyTotals } from "@/lib/db.server";

export const Route = createFileRoute("/api/v1/stats/daily-totals")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const items = await queryDailyTotals(machineId, range.from, range.to);
